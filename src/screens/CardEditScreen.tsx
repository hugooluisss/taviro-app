...
export default function CardEditScreen() {
  ...
  return <ScrollView style={{backgroundColor: theme.background}} contentContainerStyle={styles.content}>
    ...
    <View style={styles.row}><Pressable style={[styles.button, styles.shareButton, {backgroundColor: theme.accent}]} onPress={() => navigation.navigate('ShareQr', {cardId})}><Text style={styles.buttonText}>QR</Text></Pressable><Pressable style={[styles.button, styles.shareButton, {backgroundColor: theme.accent}]} onPress={() => navigation.navigate('ShareNfc', {cardId})}><Text style={styles.buttonText}>NFC</Text></Pressable><Pressable style={[styles.button, styles.shareButton, {backgroundColor: theme.accent}]} onPress={() => navigation.navigate('ShareWhatsapp', {cardId})}><Text style={styles.buttonText}>WhatsApp</Text></Pressable></View>
  </ScrollView>;
}

const styles = StyleSheet.create({loader: {flex: 1}, content: {padding: 20, gap: 12}, titleInput: {fontSize: 28, fontWeight: '700', borderBottomWidth: 1, paddingVertical: 8}, sectionTitle: {fontSize: 18, fontWeight: '700', marginTop: 8}, card: {padding: 14, borderWidth: 1, borderRadius: 12, gap: 8}, label: {fontSize: 15, borderBottomWidth: 1, paddingVertical: 6}, input: {fontSize: 17, borderBottomWidth: 1, paddingVertical: 8}, row: {flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 8}, smallButton: {padding: 8, borderRadius: 8, flex: 1, alignItems: 'center'}, button: {padding: 14, borderRadius: 10, alignItems: 'center'}, shareButton: {flex: 1}, buttonText: {color: '#FFF', fontWeight: '700'}, photo: {width: 120, height: 120, borderRadius: 8}});
