...
export default function CardEditScreen() {
  ...
  return <GradientBackground><ScrollView contentContainerStyle={styles.content}>
    ...
    {card.fields.map(field => <View key={field.id} style={[styles.card, {backgroundColor: theme.cardOverlay, borderColor: theme.cardOverlayBorder}]}>
      ...
      <TextInput accessibilityLabel={t('cardEdit.fieldValueA11y', {label: field.label})} value={field.value} onChangeText={value => updateField(field.id, {value})} placeholder={field.type === 'photo' ? t('cardEdit.photoPlaceholder') : t('cardEdit.valuePlaceholder')} placeholderTextColor={theme.onGradientMuted} style={[styles.input, {color: theme.onGradientText, borderColor: theme.cardOverlayBorder}]} />
